  ...
  //ui(new Ui::imdb)
{
    ...
    connect(this,SIGNAL(reRun()),this,SLOT(doReRun()));
...
QString &imdb::safeMovieDescr(const QString &value)
{
    ...
    return m_dropApostrophe? *new QString(result.replace("[\\/|\\\\|\\:|\\*|\\?|\\""|\\<|\\>/g]", " ").replace(QRegExp("[\\s+/g]"), " ").replace('.',' ').replace('~','-').replace('_',' ').remove(QRegExp("[^a-zA-Z\\d\\w\\s\\-\\&]")).simplified()) :  *new QString(result.replace("[\\/|\\\\|\\:|\\*|\\?|\\""|\\<|\\>/g]", " ").replace(QRegExp("[\\s+/g]"), " ").replace('.',' ').replace('~','-').replace('_',' ').replace('`','\'').remove(QRegExp("[^a-zA-Z\\d\\w\\s\\-\\&\\']")).simplified());
}
                ...
                if(!hasTables.isNull()){
                    ...
                    if(!hasAnchors.isNull()){
                        ...
                        foreach(QWebElement element, anchors){
                            QString anchorVal = element.evaluateJavaScript("this.innerText").toString();
                            if (!anchorVal.isNull() && !anchorVal.isEmpty()){
                                QString anchorText = safeMovieDescr(anchorVal);//.simplified().trimmed().remove(QRegExp("[^a-zA-Z\\d\\s]"));
...
void imdb::doRefreshCategoryTabs()
{
    ...
    if (dRoot.exists()){
        ...
        while (it.hasNext()) {
            ...
            if (it.fileInfo().isDir() && (p.cdUp() && p!=dRoot) ){
                if (!it.fileInfo().fileName().startsWith('.',Qt::CaseInsensitive) && !it.fileInfo().fileName().endsWith('/',Qt::CaseInsensitive)  )
                {
                    ...
                    if (tmp.contains("unkown",Qt::CaseInsensitive)){
                        tmp.remove(0,dir.length()+7);
                    } else {
        ...
        for(int i=0;i<m_movie_categories.count();i++){
            ...
            if (!(m_filefilter.isNull()||m_filefilter.isEmpty())){
                QString m_filter = fileMaskMatch();
                for (int j=0;j<catItem.second->count();j++){
                    QString sfolder = catItem.second->at(j);
                    QDir adir(sfolder);
                    QString fileNameMatch = QString("%1.*").arg((m_filter.isNull() || m_filter.isEmpty())?"*":m_filter.endsWith('*')?m_filter:QString("%1*").arg(m_filter));
                    QStringList files = adir.entryList(QStringList(fileNameMatch),
                                                       QDir::Files | QDir::NoSymLinks );

                    if (!files.isEmpty()){
                        tabCategoryGroups->addGallery(QString("%1 (%2)").arg(catItem.first).arg(catItem.second->count()),fileMaskMatch(),sz,*catItem.second);
...
void imdb::doSearchForTextchanged(const QString  &text)
{
    m_filefilter = text;
}

void imdb::doSearchForEditingFinished()
{
    emit reRun();
}
    ...
    if (dRoot.exists()){
        ...
        while (it.hasNext()) {
            ...
            if (it.fileInfo().isDir() && (p.cdUp() && p!=dRoot) ){
                if (!it.fileInfo().fileName().startsWith('.',Qt::CaseInsensitive) && !it.fileInfo().fileName().endsWith('/',Qt::CaseInsensitive)  )
                {
                    matchImgToMovie(QDir::cleanPath(sfolder));
                }
            }
        }
    }
...
void imdb::matchImgToMovie(const QString &dir){
...
const QString imdb::fileMaskMatch()
{
    ...
    //QString stxt = static_cast<MainWindow*>(p->centralWidget());
    //QMainWindow* p = static_cast<QMainWindow*>(parent);
    //   p->
    return (m_filefilter.isEmpty() || m_filefilter.isNull() ? "*" : m_filefilter);
}
